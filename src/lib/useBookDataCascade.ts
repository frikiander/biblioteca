 ...
 * 
 * Manages loading, rate-limit detection, errors, and merged book data state.
 */
export function useBookDataCascade(): UseBookDataCascadeResult {
  ...
  const search = useCallback(async (rawIsbn: string): Promise<BookData | null> => {
    ...
    try {
      ...
      setData(result);
      if (!result) {
        setError(`No se encontraron registros para el ISBN "${rawIsbn}" en los catálogos en línea.`);
      }
      return result;
    } catch (err: unknown) {
